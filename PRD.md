...
- [x] Playtest feedback pass (paper notes, 2026-07-05): HUD pause button (⏸/▶); green guide crumbs cut to ~25%; all four mazes regenerated 20% smaller (41→33) via randomized Prim's — many more junctions/dead ends, more moving hazards/traps, more edible biscuit walls, few loops braided in, BFS-validated solvable; teleportation gems removed (mouse holes remain); uniform touch-button sizes; map mode pinch-zoom/drag-pan (wheel+drag on desktop); fewer hearts (Kid 3 / Medium 3 / Hard 2); cat wears a collar bell that jingles louder as it gets near; overlay icons bundled via imports (fixes missing Alice image in production); push blocks show four orange direction arrows on top
